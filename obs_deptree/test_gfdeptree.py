import pytest
from obs_deptree.gfdeptree import GFDepTree
import shutil
...
class MedTree:
    def __init__(self, tempdir):
        clist = GFDepTree.expand_glob_to_nodes('c*', filepath=tempdir)
        b1 = GFDepTree(children=clist, filepath=Path(tempdir,'b1'))
        b2 = GFDepTree(filepath=Path(tempdir,'b2'))
        self.root = GFDepTree([b1, b2], filepath=Path(tempdir, 'root'))
...
def threedeep(request,sessiondir):
    root = GFDepTree.from_dict_tree(med_tree_literal, filedir=sessiondir, expand_leaves=False)
    return root
...
def test_globnodes(sessiondir):
    fdlist = GFDepTree.expand_glob_to_nodes(['c*'], sessiondir)
    assert len(fdlist) == 2
